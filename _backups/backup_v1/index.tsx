        ...
        if (navigator.share) {
            try {
                ...
                if ((error as any).name !== 'AbortError') {
                     app.showToast("Ocorreu um erro ao compartilhar.");
                }
            }
        } else {
            try {
            ...
            <div className="relative w-full lg:flex-1 lg:min-w-0 h-full flex flex-col bg-background-light dark:bg-background-dark shadow-2xl overflow-hidden transform-gpu" style={{contain: 'strict'}}>
                    ...
                    <div className="relative z-10 flex h-24 items-center justify-between gap-4 p-4">
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <div className="relative">
                                ...
                                {showProfileBadge && (
                                    <button onClick={handleProfileClick} className="absolute -top-1 -right-1 bg-red-600 text-white text-[10px] font-bold rounded-full h-5 w-5 flex items-center justify-center shadow-sm border border-white z-20 animate-bounce cursor-pointer">
                ...
                {app.isCartTooltipVisible && showSessionBar && (
                    <div className={`absolute right-4 z-[130] w-auto max-w-[200px] bg-zinc-800/95 backdrop-blur-md border border-white/10 text-white rounded-xl shadow-2xl animate-fadeIn p-3 pointer-events-none transition-all duration-300 ${app.isFocusMode ? 'top-16' : 'top-40 lg:top-16'}`}>
                        ...
                        <p className="text-[10px] text-gray-300 leading-tight">Seus itens marcados estão salvos. Finalize a compra para limpar a lista.</p>
                ...
                <footer className="fixed lg:hidden bottom-0 w-full z-[100] bg-white/70 dark:bg-[#121212]/70 backdrop-blur-xl border-t border-white/20 dark:border-white/10 shadow-[0_-10px_20px_-5px_rgba(0,0,0,0.1)] pb-safe-area transition-all duration-300">
                    <div className="flex items-end justify-between px-2 h-16 w-full max-w-full">
                        <div className="flex-1 h-full flex items-center justify-center">
                            <NavButton icon="home" label="Início" onClick={() => app.setHomeViewActive(true)} active={app.isHomeViewActive} />
                        </div>
                        <div className="flex-1 h-full flex items-center justify-center">
                            <NavButton icon="favorite" label="Favoritos" onClick={() => { if (!user) { app.showToast("Faça login para ver favoritos"); app.openModal('auth'); } else { app.openModal('favorites'); } }} active={app.isFavoritesModalOpen} />
                        </div>
                        <div className="flex-1 h-full flex items-center justify-center relative">
                            <button onClick={() => { if (app.isHomeViewActive) { if (items.length > 0 || app.currentMarketName) { app.setHomeViewActive(false); } else { app.openModal('startShopping'); } } else { if (items.length === 0) app.openModal('startShopping'); else app.openModal('addItem'); } }} className={`absolute bottom-4 flex h-16 w-16 items-center justify-center rounded-full text-white shadow-xl ring-4 ring-white/50 dark:ring-black/20 backdrop-blur-sm transition-all active:scale-95 ${app.theme === 'christmas' ? 'bg-[#165B33]' : (app.theme === 'newyear' ? 'bg-amber-500' : 'bg-gradient-to-br from-primary to-orange-600')}`} title={app.isHomeViewActive ? "Ver minha lista" : "Adicionar item"}>
                        ...
                        <div className="flex-1 h-full flex items-center justify-center">
                            <NavButton icon="local_offer" label="Ofertas" onClick={() => app.openModal('offers')} active={app.isOffersModalOpen} />
                        </div>
                        <div className="flex-1 h-full flex items-center justify-center">
                            <div className="relative">
                                <NavButton icon={<span className="material-symbols-outlined text-[28px] animate-color-pulse">auto_awesome</span>} label="Menu" onClick={handleBottomMenuClick} active={app.isToolsModalOpen} />
                            </div>
                        </div>
                    </div>
                </footer>
