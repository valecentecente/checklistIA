            ...
            <div className="relative w-full max-w-sm overflow-hidden rounded-3xl bg-background-light dark:bg-surface-dark shadow-2xl flex flex-col animate-slideUp" onClick={(e) => e.stopPropagation()}>
                ...
                <div className="flex flex-col p-6 space-y-4">
                    <form onSubmit={handleSetBudget}>
                        <h2 className="text-slate-800 dark:text-gray-100 tracking-tight text-2xl font-black text-center font-display">Meu Orçamento</h2>
                        <p className="text-slate-500 dark:text-gray-400 text-sm font-medium leading-tight text-center mt-1">Qual o limite para gastar nesta compra?</p>
                        ...
                        <div className="flex flex-wrap items-end gap-4 py-4">
                            <label className="flex flex-col min-w-40 flex-1">
                                <div className="relative">
                                    ...
                                    <input
                                        className="form-input flex w-full min-w-0 flex-1 resize-none overflow-hidden rounded-2xl text-2xl font-black text-slate-800 dark:text-white border-2 border-gray-100 dark:border-gray-800 bg-gray-50 dark:bg-black/20 focus:border-primary focus:ring-0 h-16 pl-12 pr-4 text-right shadow-inner"
                                        placeholder="0,00"
                                        type="text"
                                        inputMode="numeric"
                                        value={inputValue}
                        ...
                        <div className="bg-gray-100 dark:bg-black/20 rounded-2xl p-3 border border-gray-200 dark:border-gray-800 mb-4">
                            <p className="text-[9px] font-black text-slate-400 dark:text-gray-500 uppercase tracking-widest mb-2 text-center">Indicador de Gasto</p>
                            <div className="grid grid-cols-2 gap-y-2 gap-x-4">
                                <div className="flex items-center gap-2">
                                    <div className="h-2 w-2 rounded-full bg-green-500"></div>
                                    <span className="text-[10px] font-bold text-slate-600 dark:text-gray-400">No Orçamento</span>
                                </div>
                                <div className="flex items-center gap-2">
                                    <div className="h-2 w-2 rounded-full bg-yellow-500"></div>
                                    <span className="text-[10px] font-bold text-slate-600 dark:text-gray-400">Atenção (80%)</span>
                                </div>
                                <div className="flex items-center gap-2">
                                    <div className="h-2 w-2 rounded-full bg-red-500"></div>
                                    <span className="text-[10px] font-bold text-slate-600 dark:text-gray-400">No Limite (95%)</span>
                                </div>
                                <div className="flex items-center gap-2">
                                    <div className="h-2 w-2 rounded-full bg-red-600 animate-pulse"></div>
                                    <span className="text-[10px] font-bold text-slate-600 dark:text-gray-400">Estourado!</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="flex flex-col gap-2 pt-2">
                             <div className="grid grid-cols-2 gap-2">
                                <button type="button" onClick={onClose} className="flex w-full cursor-pointer items-center justify-center rounded-xl h-14 bg-gray-100 dark:bg-white/5 text-slate-500 dark:text-gray-400 text-sm font-bold transition-all active:scale-95">
                                    <span>Cancelar</span>
                                </button>
